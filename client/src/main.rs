...
const MEMORY_DUMP_INTERVAL_SECONDS: u64 = 40;
...
/// # Returns
/// * `Vec<[u8; 32]>` - Vector of 32-byte leaf hashes
fn generate_leaves_from_file(mem_path: &str) -> Result<Vec<[u8; 32]>, Box<dyn std::error::Error>> {
...
fn profile_memory_changes(process_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Static memory regions: {} out of {}", static_count, is_static.len());
    
    // Create compressed static memory dump
    let static_memory: Vec<u8> = initial_leaves.iter().zip(is_static.iter())
        .flat_map(|(&byte, &is_static)| if is_static { byte.to_vec() } else { vec![0; 32] })
        .collect();
...
fn profile_nes_memory() -> Result<(), Box<dyn std::error::Error>> {
    let initial_leaves = generate_leaves_from_file("/tmp/nes_memory_dump_0.bin")?;
    ...
    for dump_index in 1..10 {
        ...
        let current_leaves = generate_leaves_from_file(&dump_path)?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while running {
        ...
        let tree = generate_leaves_from_file("/tmp/nes_memory_dump_proof.bin")?;
